...
public record TopicItem
{
    ...
    public required PushItem Item { get; init; }
...
public class TopicActor : Actor, ITopicActor, IRemindable
{
    ...
    /// <summary>
    /// Whether a subscriber is currently eligible for a Push attempt this tick (not blacklisted,
    ...
    private async Task<PushResponse> PushWithTimeoutAsync(string subscriberId, PushRequest pushRequest)
    {
        ...
        try
        {
            var pushTask = _queueActorInvoker.InvokeMethodAsync<PushRequest, PushResponse>(actorId, "Push", pushRequest);
            var completed = await Task.WhenAny(pushTask, Task.Delay(timeout));

            if (completed != pushTask)
            {
                return new PushResponse { Success = false, ItemsPushed = 0, ErrorMessage = "Relay push timed out" };
            }
        ...
        {
            return new PushResponse { Success = false, ItemsPushed = 0, ErrorMessage = ex.Message };
        }
    }
    ...
    private async Task RelayTickAsync()
    {
        ...
        var batches = new Dictionary<string, List<PushItem>>();
        ...
        var pushTasks = batches.ToDictionary(
            kvp => kvp.Key,
            kvp => PushWithTimeoutAsync(kvp.Key, new PushRequest { Items = kvp.Value }));
        ...
        foreach (var subscriberId in eligibleSubscribers)
        {
            ...
            {
                // Nothing to push (empty or fully non-member batch) - cursor still advances past
                // whatever was skipped, no Push call needed.
                updatedCursors[subscriberId] = advancedCursors[subscriberId];
                continue;
            }

            bool success = pushTasks[subscriberId].Result.Success;
        ...
        int nextInterval = pushTasks.Count > 0
            ? 1
