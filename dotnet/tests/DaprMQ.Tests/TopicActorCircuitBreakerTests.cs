...
public class TopicActorCircuitBreakerTests
{
    private static Mock<IQueueActorInvoker> CreateFailingInvoker(string subscriberSuffix)
    {
        ...
        mock.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
                It.Is<ActorId>(id => id.GetId().EndsWith(subscriberSuffix)),
                ActorMethodNames.Push, It.IsAny<PushRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PushResponse { Success = false, ItemsPushed = 0, ErrorMessage = "down" });
        return mock;
    }
    ...
    public async Task OneHourElapsedSinceFirstFailure_Blacklists_NoFurtherAttempts()
    {
        ...
        // Further ticks must not attempt Push at all for a blacklisted subscriber.
    ...
    public async Task SingleSuccess_FullyClearsCircuitState()
    {
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
            ...
            .ReturnsAsync(() =>
            {
                callCount++;
                return callCount == 1
                    ? new PushResponse { Success = false, ItemsPushed = 0, ErrorMessage = "down" }
    ...
    public async Task ResetCircuitBreaker_OnBlacklisted_DeletesState_NextTickRetriesFromUnchangedCursor()
    {
        ...
        successInvoker.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
                It.IsAny<ActorId>(), ActorMethodNames.Push, It.IsAny<PushRequest>(), It.IsAny<CancellationToken>()))
        ...
        await actor2.Publish(new PublishRequest { Items = new List<PushItem> { new PushItem { ItemJson = "{}" } } });
