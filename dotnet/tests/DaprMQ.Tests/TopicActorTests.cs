...
public class TopicActorTests
{
    ...
    public async Task Publish_WritesItemsWithStrictlyIncreasingSequenceAndCurrentGeneration_DoesNotTouchCursors()
    {
        ...
        var response = await actor.Publish(new PublishRequest
        {
            ...
            {
                new PushItem { ItemJson = "{\"a\":1}" },
                new PushItem { ItemJson = "{\"a\":2}" }
            }
        });
    ...
    public async Task Publish_EmptyItems_ReturnsNotAccepted()
    {
        ...
        var response = await actor.Publish(new PublishRequest { Items = new List<PushItem>() });
    ...
    public async Task RelayTick_TwoSubscribers_OneFailsOneSucceeds_OnlySuccessfulCursorAdvances_SingleSave()
    {
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
                ...
                It.IsAny<PushRequest>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PushResponse { Success = false, ItemsPushed = 0, ErrorMessage = "boom" });
    ...
    public async Task RelayTick_ExcludesItemFromSubscriberWhoseGenerationPredatesIt()
    {
        ...
        List<PushItem>? pushedToLate = null;
        mockInvoker.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
                It.IsAny<ActorId>(),
                ActorMethodNames.Push,
                It.IsAny<PushRequest>(),
                It.IsAny<CancellationToken>()))
            .Callback<ActorId, string, PushRequest, CancellationToken>((id, _, req, _) =>
            {
                if (id.GetId() == "test-topic-sub-late") pushedToLate = req.Items;
            })
            .ReturnsAsync(new PushResponse { Success = true, ItemsPushed = 1 });
        ...
        await actor.Publish(new PublishRequest { Items = new List<PushItem> { new PushItem { ItemJson = "{\"n\":1}" } } });
        await actor.Subscribe(new SubscribeRequest { SubscriberId = "late" });
        await actor.Publish(new PublishRequest { Items = new List<PushItem> { new PushItem { ItemJson = "{\"n\":2}" } } });
        ...
        Assert.NotNull(pushedToLate);
        Assert.Single(pushedToLate!);
        Assert.Equal("{\"n\":2}", pushedToLate![0].ItemJson);
    }

    [Fact]
    public async Task RelayTick_FullyNonMemberBatch_SkipsPushCallButAdvancesCursor()
    {
        var stateData = new Dictionary<string, object>();
        var mockStateManager = CreateMockStateManager(stateData);
        var mockInvoker = new Mock<IQueueActorInvoker>();
        ...
        mockInvoker.Verify(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
            It.IsAny<ActorId>(), ActorMethodNames.Push, It.IsAny<PushRequest>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    [Fact]
    public async Task RelayTick_PushNeverCompletes_TreatedAsFailedNotAwaitedIndefinitely()
    {
        var stateData = new Dictionary<string, object>();
        var mockStateManager = CreateMockStateManager(stateData);
        var mockInvoker = new Mock<IQueueActorInvoker>();
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<PushRequest, PushResponse>(
            ...
            .Returns(new TaskCompletionSource<PushResponse>().Task); // never completes
