...
│   ├── EnqueueSection.tsx  # Priority-based enqueue form
│   ├── DequeueSection.tsx  # Dequeue/DequeueLocked controls
...
**Endpoints:**
- `POST /queue/{id}/enqueue` - Enqueue message
- `POST /queue/{id}/dequeue` - Dequeue (immediate removal)
- `POST /queue/{id}/dequeue` (header: `require-ack: true`) - Dequeue with lock
...
**TTL:** 30 seconds (hardcoded in `DequeueLocked`)
...
**Lock expired errors:** Expected after 30s TTL, user should dequeue new message
