...
- DequeueLocked: Register reminder `await RegisterReminderAsync($"lock-{lockId}", TimeSpan.FromSeconds(ttl))`
...
- Remove manual expiration checks from DequeueLocked, Acknowledge
...
- DequeueLocked: Dequeue item, store in lock state (decrement queue count)
...
**Key Code:**
```csharp
// In DequeueLocked after peeking
var (itemJson, priority, headSegment) = await DequeueWithPriorityAsync(); // Now actually dequeues
...
**Tests:**
- Enqueue 10 → DequeueLocked 5 → queue count = 5 (not 10)
...
**Changes:**
- Add `bool AllowCompetingConsumers` to `DequeueLockedRequest`
- Add `bool CompetingConsumerMode` to `LockState`
- Replace `_current_lock_id` (string) with `_lock_registry` (List<string>)
- DequeueLocked: If `AllowCompetingConsumers=false`, block if registry not empty (legacy)
- DequeueLocked: If `AllowCompetingConsumers=true`, create lock without blocking (new)
...
{
    return new DequeueLockedResponse { Locked = true };
}
...
**Tests:**
- Two DequeueLocked (`AllowCompetingConsumers=true`) both succeed with different items
