...
**Architecture:** The HTTP Sink entity is essentially a Client of the Queue.  HTTP Sink uses Dapr Actor Reminders to periodically poll the Queue via `DequeueLocked`, then delivers batches of messages via HTTP POST. 

## Architecture Overview

```
Enqueue Message → QueueActor (no sink knowledge)
...
Dapr Actor Reminder → SinkActor.ReceiveReminderAsync("sink-poll")
                      ...
                      PollAndDeliver()
                          ↓
        DequeueLocked(Count=100, MaxConcurrency=M) → QueueActor
                          ↓
                    Items with locks
                          ↓
...
3. **MaxConcurrency Control**: SinkActor passes `MaxConcurrency` parameter to `DequeueLocked` - QueueActor returns `min(Count, MaxConcurrency - LockCount)` items
...
public record SinkActorState
{
    ...
    public required int LockTtlSeconds { get; init; }              // Lock TTL for DequeueLocked
...
### DequeueLockedRequest Enhancement (Models.cs)
...
### DequeueLockedResponse Enhancement (Models.cs)
...
- `DequeueLocked()` - Added MaxConcurrency enforcement:
  ```csharp
...
- **`PollAndDeliver()`** - Core polling logic:
  ...
  2. Call DequeueLocked on QueueActor:
  ...
  5. Serialize dequeueResult.Items as JSON (camelCase)
  6. POST to state.Url
  7. If HTTP 200 OK:
...
- `POST /queue/{queueId}/sink/http/register` - Initialize SinkActor with polling configuration
- `POST /queue/{queueId}/sink/http/unregister` - Uninitialize SinkActor (stops polling)
...
1. `DequeueLocked_WithMaxConcurrency_ReturnsAvailableCapacity` - Verify MaxConcurrency enforcement
2. `DequeueLocked_WithMaxConcurrencyAtLimit_ReturnsMaxConcurrencyReached` - Verify capacity limit response
...
6. `PollAndDeliver_WithItems_CallsDequeueLocked` - Verify DequeueLocked invocation
...
6. `ConcurrencyLimit_EnforcedByDequeueLocked` - Enqueue 10 items with MaxConcurrency=3, verify only 3 delivered
7. `PollingInterval_Configurable` - Verify 1s, 5s, 10s intervals work correctly

## Verification

1. **Unit tests pass** - `cd server && dotnet test` (✅ 111 passed, 0 failed)
...
3. **Manual testing:**
   ...
   curl -X POST http://localhost:5000/queue/test-queue/sink/http/register \
     -H "Content-Type: application/json" \
     -d '{"url":"http://localhost:8080/webhook","maxConcurrency":5,"lockTtlSeconds":30,"pollingIntervalSeconds":2}'

   # Enqueue messages
   curl -X POST http://localhost:5000/queue/test-queue/enqueue \
     -H "Content-Type: application/json" \
     -d '{"items":[{"itemJson":"{\"test\":\"data1\"}","priority":1},{"itemJson":"{\"test\":\"data2\"}","priority":1}]}'

   # Wait 2-3 seconds - SinkActor reminder will fire
   # Verify HTTP POST received at localhost:8080 with array of DequeueLockedItem objects
   # Expected body: [{"itemJson":"...","lockId":"...","priority":1,"lockExpiresAt":...}, ...]

   # Unregister sink
   curl -X POST http://localhost:5000/queue/test-queue/sink/http/unregister
...
- **MaxConcurrency enforcement**: QueueActor's `DequeueLocked` calculates `min(Count, MaxConcurrency - LockCount)`
...
3. **MaxConcurrency via DequeueLocked** - SinkActor passes `MaxConcurrency` parameter to `DequeueLocked`, QueueActor enforces limit
