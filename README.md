...
This is as simple as it gets. Use `enqueue` to add a message to the Queue. Use `dequeue` to dequeue a message in your processing loop.

```bash
# Enqueue a message to the queue with priority 1
curl -X POST http://localhost:8002/queue/my-queue/enqueue \
  -H "Content-Type: application/json" \
  -d '{ "items": [{ "item": { "task": "first" }, "priority": 1 }] }'

# Enqueue a message to the same queue, with priority 0
curl -X POST http://localhost:8002/queue/my-queue/enqueue \
  -H "Content-Type: application/json" \
...
- **It just works!**: Two endpoints to get started (`Enqueue`, `Dequeue`) - that's it.
...
- **Acknowledgements / at-least-once delivery**: Consumers can optionally specify that `Dequeue` requires a follow-on `Ack` - If the `Ack` is not received within a timeout period, the message is made available at the next `Dequeue`.
- **Bulk Enqueue & Bulk Dequeue**: Atomic operations for enqueuing and dequeuing many items, for best throughput.
- **Competing Consumers**: Have many dequeue operations concurrently in a traditional competing consumer pattern. This requires the use of Acknowledgements. **FIFO is not gauranteed when using competing consumers.**
- **Dead-lettering**: When using Acknowledgements, you can call `/dead-letter` to move the poison message to a dedicated deadletter queue, allowing subsequent `dequeue` operations to continue
- **On-Demand**: Queues are created on-demand at the time of first `Dequeue` or `Enqueue`, there is no need to create them AOT.
...
- Kafka has no built-in concept of priority messaging, which once again, is very reasonable position given it is a Streaming technology built ontop of Partitions. There are several strategies to accomplish a priority messaging concept, but they are not lightweight. e.g. Building ones own priority system atop of Kafka Topics, or repartitioning the streams with middleware etc.
  ...
  **Each daprMQ has a virtually-unlimited amount of sub-queues which represent priority. Messages are by default enqueued into a Priority 1 sub-queue, but the range is from 0 - 2,147,483,647. Application Developers have complete control over the priority when *Enqueuing* a message.**

## Roadmap

### Done

- [x] **Bulk Enqueue**: Write many messages to a queue in one atomic operation.
- [x] **Bulk Dequeue**: Dequeue many message from a queue in one atomic operation.
- [x] **Competing Consumers**: Experimental project (requires Bulk Enqueue and Bulk Dequeue as a pre-requisite)
...
- [ ] **Max Retries / Automatic Dead-lettering**: When calling Dequeue With Acknowledgements, if a message is dequeued N number of times without Acknowledgement, the message will be deemed poisonous, and automatically moved to the Deadletter queue.
...
- [ ] **Scheduled Enqueue**: A message can be enqueued to the front of any sub-queue, at a future scheduled time (Requires Message Receipts feature)
- [ ] **Message revocation**: A message can be removed from the queue, regardless of its position (Requires Message Receipts feature)
- [ ] **Rate-limit**: Enqueue and Dequeue operations are protected with a per queue rate-limiter.
- [ ] **Qoutas**: Queues are governed by Enqueue and Dequeue qouates per minute/hour/day.
- [ ] **Queue capacity limit**: Prevent Enqueues until queue length drops below a capacity limit.
- [ ] **Queue reorder functions**: Reorder a queue based on a field within the Json.
- [ ] **Message functions**: Before a Dequeue is about to occur, call a HTTP endpoint with the message payload and replace message payload with the HTTP response.
...
- [ ] **Long-polling Dequeue**: Dequeue can optionally hold the request open until a message is available or a timeout elapses, instead of requiring the client to poll on an interval.
...
- [ ] **Bulk Acknowledge**: Acknowledge many locked messages from a Dequeue With Acknowledgement in one atomic operation, mirroring Bulk Enqueue and Bulk Dequeue.
...
Dapr Actors are extremely lightweight. Therefore hundreds of thousands of them can be active in-memory keeping the latencies of `Enqueue` and `Dequeue` operations as low as possible. 

Each `Enqueue` or `Dequeue` operation will create just a single transactional operation to the underlying persistent store, and often times will be a just 2 database operations in that transaction. 
...
State is stored as: `queue_0_seg_0`, `queue_0_seg_1`, `queue_1_seg_0`, etc., plus a `metadata` map with segment pointers. Dequeue operations drain from priority 0 completely before moving to priority 1, and so on.
...
- `Dequeue()`: O(k) where k = number of priority levels with items
...
**I/O Operations:**
- Enqueue: 2 state store operations (segment + metadata)
- Dequeue: 2 state store operations (segment + metadata)
