...
public record BlobReaperActorState
{
    ...
    /// can tell whether a requested delay would enqueue the deletion later or earlier.
    /// </summary>
...
public class BlobReaperActor : Actor, IBlobReaperActor, IRemindable
{
    ...
    /// <summary>
    /// Enqueues the scheduled deletion out to fire NewDelaySeconds from now, but only if that's
    ...
    public async Task PostponeDeletion(PostponeDeletionRequest request)
    {
        ...
        {
            Logger.LogDebug(
                "BlobReaperActor {ActorId} PostponeDeletion no-op - new delay would not enqueue deletion of {BlobReference} later",
                Id.GetId(), request.BlobReference);
            return;
        }
