...
Manual performance testing tool to compare HTTP and gRPC enqueue operation latencies.
...
dotnet run --project server/tests/DaprMQ.PerformanceTests
```
...
- 5 HTTP clients run concurrently, each with its own queue, each doing 1000 enqueue operations = 5000 total HTTP operations
- 5 gRPC clients run concurrently, each with its own queue, each doing 1000 enqueue operations = 5000 total gRPC operations
...
```
=== Enqueue Operation Performance Test ===
...
Testing HTTP Enqueue (5 virtual users × 1000 iterations = 5000 total operations)...
Testing gRPC Enqueue (5 virtual users × 1000 iterations = 5000 total operations)...
