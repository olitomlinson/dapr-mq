...
Console.WriteLine("=== Enqueue Operation Performance Test ===");
...
{
    await EnqueueViaHttp(warmupHttpClient, httpQueueIds[0], testJson, priority: 1);
}

for (int i = 0; i < warmupIterations; i++)
{
    await EnqueueViaGrpc(warmupGrpcClient, grpcQueueIds[0], testJson, priority: 1);
}
...
Console.WriteLine($"Testing HTTP Enqueue ({virtualUsers} virtual users × {testIterations} iterations = {virtualUsers * testIterations} total operations)...");
...
var httpUserTasks = Enumerable.Range(0, virtualUsers).Select(async userId =>
{
    ...
    for (int i = 0; i < testIterations; i++)
    {
        ...
        var (success, statusCode) = await EnqueueViaHttp(httpClient, queueId, testJson, priority: 1);
        sw.Stop();
...
Console.WriteLine($"Testing gRPC Enqueue ({virtualUsers} virtual users × {testIterations} iterations = {virtualUsers * testIterations} total operations)...");
...
var grpcUserTasks = Enumerable.Range(0, virtualUsers).Select(async userId =>
{
    ...
    for (int i = 0; i < testIterations; i++)
    {
        var requestStart = Stopwatch.GetTimestamp();
        var sw = Stopwatch.StartNew();
        var (success, statusCode) = await EnqueueViaGrpc(grpcClient, queueId, testJson, priority: 1);
        sw.Stop();
        // Timestamp relative to when this user started (not global start)
        var elapsedSeconds = (requestStart - userStartTime) / (double)Stopwatch.Frequency;
        // Add user's start delay to shift timeline
...
Console.WriteLine("HTTP Enqueue:");
...
// Helper methods
static async Task<(bool success, int statusCode)> EnqueueViaHttp(HttpClient client, string queueId, string itemJson, int priority)
{
    try
    {
        var request = new
        ...
        var response = await client.PostAsJsonAsync($"/queue/{queueId}/enqueue", request);
...
static async Task<(bool success, int statusCode)> EnqueueViaGrpc(GrpcService.DaprMQClient client, string queueId, string itemJson, int priority)
{
    try
    {
        ...
        request.Items.Add(new EnqueueItem { ItemJson = itemJson, Priority = priority });
...
static string GeneratePerformanceGraph(List<(double timestamp, double latency, bool success, int statusCode)> httpTimings, List<(double timestamp, double latency, bool success, int statusCode)> grpcTimings, string resultsDir, int virtualUsers, double rampUpRate)
{
    ...
    plot.Title($"HTTP vs gRPC Enqueue Performance ({userText}{rampUpText}, 1s buckets)");
