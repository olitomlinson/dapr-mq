...
public class TopicControllerTests
{
    ...
    private static ApiPublishRequest SinglePublishRequest() => new(new List<ApiEnqueueItem>
    {
        new ApiEnqueueItem(JsonSerializer.SerializeToElement(new { hello = "world" }))
    });
    ...
    [Fact]
    public async Task Publish_WithIdempotencyKeyOnItem_MapsToActorEnqueueItem()
    {
        var mockInvoker = new Mock<ITopicActorInvoker>();
        ...
        var controller = CreateController(mockInvoker.Object);
        var request = new ApiPublishRequest(new List<ApiEnqueueItem>
        {
            new ApiEnqueueItem(JsonSerializer.SerializeToElement(new { hello = "world" }), IdempotencyKey: "topic-key-1")
        });
    ...
    public async Task Publish_EmptyItems_Returns400()
    {
        ...
        var result = await controller.Publish("topic-a", new ApiPublishRequest(new List<ApiEnqueueItem>()));
