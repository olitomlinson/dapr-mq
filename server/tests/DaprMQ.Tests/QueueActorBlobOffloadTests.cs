...
public class QueueActorBlobOffloadTests
{
    ...
    [Fact]
    public async Task Dequeue_BlobRefItem_PopulatesBlobReferenceAndSchedulesReaping()
    {
        var mockStateManager = CreateMockStateManager();
        var (actor, reaperInvoker) = await CreateActorAsync(mockStateManager);

        await actor.Enqueue(new EnqueueRequest { Items = new List<EnqueueItem> { new() { ItemJson = BlobRef("blob-1"), Priority = 0 } } });
    ...
    [Fact]
    public async Task Dequeue_NormalItem_LeavesObjectClaimTokenNullAndDoesNotSchedule()
    {
        var mockStateManager = CreateMockStateManager();
        var (actor, reaperInvoker) = await CreateActorAsync(mockStateManager);

        await actor.Enqueue(new EnqueueRequest { Items = new List<EnqueueItem> { new() { ItemJson = "{\"a\":1}", Priority = 0 } } });
        var result = await actor.Dequeue(new DequeueRequest());
    ...
    [Fact]
    public async Task DequeueLocked_BlobRefItem_PopulatesObjectClaimTokenButDoesNotScheduleReaping()
    {
        var mockStateManager = CreateMockStateManager();
        var (actor, reaperInvoker) = await CreateActorAsync(mockStateManager);

        await actor.Enqueue(new EnqueueRequest { Items = new List<EnqueueItem> { new() { ItemJson = BlobRef("blob-2"), Priority = 0 } } });
    ...
    public async Task Acknowledge_BlobRefItem_SchedulesReaping()
    {
        ...
        await actor.Enqueue(new EnqueueRequest { Items = new List<EnqueueItem> { new() { ItemJson = BlobRef("blob-3"), Priority = 0 } } });
        var dequeueResult = await actor.DequeueLocked(new DequeueLockedRequest());
        await actor.Acknowledge(new AcknowledgeRequest { LockId = dequeueResult.Items[0].LockId });
