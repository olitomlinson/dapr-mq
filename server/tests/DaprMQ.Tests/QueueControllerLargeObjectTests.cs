...
/// <summary>
/// Unit tests for QueueController's large-object endpoints (enqueue-object, and blob dereferencing on Dequeue).
/// </summary>
public class QueueControllerLargeObjectTests
{
    ...
    private readonly Mock<ISessionCoordinatorActorInvoker> _mockSessionCoordinatorActorInvoker = new();
    ...
    [Fact]
    public async Task EnqueueObject_UploadsBodyAndEnqueuesBlobRefEnvelope()
    {
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
                It.IsAny<ActorId>(), "Enqueue", It.IsAny<EnqueueRequest>(), It.IsAny<CancellationToken>()))
        ...
        var result = await controller.EnqueueObject("test-queue");
    ...
    [Fact]
    public async Task EnqueueObject_UsesExplicitPrefixWhenProvided()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
                It.IsAny<ActorId>(), "Enqueue", It.IsAny<EnqueueRequest>(), It.IsAny<CancellationToken>()))
        ...
        var result = await controller.EnqueueObject("test-queue", prefix: "store-b");
    ...
    [Fact]
    public async Task EnqueueObject_WithIdempotencyKeyHeader_MapsToActorEnqueueItem()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        EnqueueRequest? capturedRequest = null;
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
        ...
        var result = await controller.EnqueueObject("test-queue", idempotencyKey: "object-key-456");
    ...
    [Fact]
    public async Task EnqueueObject_WithoutIdempotencyKeyHeader_LeavesKeyNull()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        EnqueueRequest? capturedRequest = null;
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
    ...
    [Fact]
    public async Task EnqueueObject_NegativePriority_ReturnsBadRequest()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        var mockObjectStore = new Mock<IObjectStore>();
        var controller = CreateController(mockInvoker, mockObjectStore);

        var result = await controller.EnqueueObject("test-queue", priority: -1);
    ...
    [Fact]
    public async Task Dequeue_SingleBlobRefItem_ReturnsJsonWithObjectClaimTokenNoInlineBytes()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        mockInvoker.Setup(i => i.InvokeMethodAsync<DequeueRequest, DequeueResponse>(
                It.IsAny<ActorId>(), "Dequeue", It.IsAny<DequeueRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new DequeueResponse
            {
                Items = new List<DequeueItem> { new() { ItemJson = "{}", Priority = 0, ObjectClaimToken = "claim-token-1" } },
                Locked = false,
                IsEmpty = false
            });
    ...
    [Fact]
    public async Task Dequeue_MultipleItemsWithOneBlobRef_ReturnsObjectClaimTokenForBlobItem()
    {
        var mockInvoker = new Mock<IQueueActorInvoker>();
        mockInvoker.Setup(i => i.InvokeMethodAsync<DequeueRequest, DequeueResponse>(
                It.IsAny<ActorId>(), "Dequeue", It.IsAny<DequeueRequest>(), It.IsAny<CancellationToken>()))
        ...
        var result = await controller.Dequeue("test-queue", require_ack: false, count: 2);
