...
public class TopicActorReaperTests
{
    ...
    public async Task ReapItems_DeletesItemsBelowMinCursor_KeepsItemsStillNeeded()
    {
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
                ...
                ActorMethodNames.Enqueue, It.IsAny<EnqueueRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new EnqueueResponse { Success = false, ItemsEnqueued = 0, ErrorMessage = "nope" });
    ...
    public async Task ReapItems_ZeroSubscribers_ReapsEverythingUpToNextSequence()
    {
        ...
        await actor.Publish(new PublishRequest { Items = new List<EnqueueItem> { new EnqueueItem { ItemJson = "{}" }, new EnqueueItem { ItemJson = "{}" } } });
        ...
        mockInvoker.Setup(i => i.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
            ...
            .ReturnsAsync(() =>
            {
                ...
                return callCount == 1
                    ...
                    : new EnqueueResponse { Success = false, ItemsEnqueued = 0, ErrorMessage = "nope" };
            });

        var actor = await CreateActorAsync(mockStateManager, mockInvoker);
        await actor.Subscribe(new SubscribeRequest { SubscriberId = "sub-a" });

        var firstPublish = await actor.Publish(new PublishRequest { Items = new List<EnqueueItem> { new EnqueueItem { ItemJson = "{}" } } });
        await actor.ReceiveReminderAsync("relay", Array.Empty<byte>(), TimeSpan.Zero, TimeSpan.FromSeconds(1));

        var secondPublish = await actor.Publish(new PublishRequest { Items = new List<EnqueueItem> { new EnqueueItem { ItemJson = "{}" } } });
        await actor.ReceiveReminderAsync("relay", Array.Empty<byte>(), TimeSpan.Zero, TimeSpan.FromSeconds(1));
