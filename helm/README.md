...
This document assumes a real target environment (an existing state store, a registry-hosted image, a namespace your organization already manages). Developing or testing the chart/server itself, or just want a throwaway local cluster to try things out? See [CONTRIBUTOR_GUIDE.md](CONTRIBUTOR_GUIDE.md).

## Recommended namespace and release name

**Use namespace `daprmq` and release name `daprmq`.** This guide, the [example apps](../examples/README.md), and [CONTRIBUTOR_GUIDE.md](CONTRIBUTOR_GUIDE.md) all assume these two values throughout - every command below uses them literally, not as placeholders. Following them means every command here is copy-pasteable as-is, and the example apps chart works against your install with zero `--set` flags.

Only deviate from `daprmq`/`daprmq` if you specifically need multiple installations in one cluster - running several environments (dev/staging/prod) or multiple tenants side by side. Those are real, supported scenarios, covered later in [Multiple Installations](#multiple-installations-example) and [Namespace Strategy](#namespace-strategy) - but they're deliberate exceptions to make only when you actually have that need, not the default starting point. If you do deviate, see the note at the end of [Quick Start](#quick-start) for what else you'll need to change to match.
...
### Recommended: PostgreSQL via the bitnami chart

```bash
kubectl create namespace daprmq

helm repo add bitnami https://charts.bitnami.com/bitnami
helm install postgres bitnami/postgresql \
  -n daprmq \
  --set auth.postgresPassword=daprmq_secret_123 \
  --set auth.database=actor_state \
  --wait
```

Replace `daprmq_secret_123` with a real secret for anything beyond a quick trial - anyone who can read Secrets in the `daprmq` namespace can otherwise see it in plain text via `--set`. This creates a `postgres-postgresql` Service in the `daprmq` namespace, which the Component CR below points at.

Already have a Postgres, Redis, or CosmosDB instance you'd rather use? Skip this step and adjust the `connectionString`/`type` in the Component CR below to match yours instead - see [Dapr's state store docs](https://docs.dapr.io/reference/components-reference/supported-state-stores/) for other store types.
...
metadata:
  name: statestore
  namespace: daprmq  # MUST match the namespace you install DaprMQ into
spec:
  type: state.postgresql
  version: v2
  metadata:
    - name: connectionString
      value: "host=postgres-postgresql.daprmq.svc.cluster.local user=postgres password=daprmq_secret_123 port=5432 database=actor_state connect_timeout=10"
...
Apply the component:
```bash
kubectl apply -f statestore.yaml -n daprmq
```
...
> **State Store Name**: The `dapr.stateStoreName` parameter is **required** and must match the name of your Dapr state store Component deployed in the same namespace (`statestore`, if you followed the steps above verbatim).

> **Using a different namespace or release name anyway?** The example apps chart won't find your gateway automatically - pass `--set daprmq.releaseName=<your-release-name>` and/or `--set daprmq.namespace=<your-namespace>` when installing `examples/helm`. See [examples/README.md](../examples/README.md) for the full override list.
...
```bash
helm install daprmq ./helm -n daprmq -f my-values.yaml
```
...
# Test enqueue operation - note "items" is a plural array, even for a single item
curl -X POST http://localhost:8080/queue/my-queue/enqueue \
  -H "Content-Type: application/json" \
  -d '{"items":[{"item":{"message":"hello"},"priority":1}]}'

# Test dequeue operation
curl -X POST http://localhost:8080/queue/my-queue/dequeue
```
...
grpcurl -plaintext -d '{"queue_id":"my-queue","item_json":"{\"message\":\"hello\"}","priority":1}' \
  localhost:8081 daprmq.DaprMQ/Enqueue
```
...
**Default recommendation:** namespace `daprmq` (see [above](#recommended-namespace-and-release-name)). The patterns below are for when you actually need multiple environments or tenants in one cluster - they're deliberate deviations from that default, not alternatives to pick from freely.
...
**Default recommendation:** release name `daprmq` (see [above](#recommended-namespace-and-release-name)). The rules below matter once you deviate from that - e.g. running multiple installations - since the release name propagates into the Dapr app-id and actor type.
...
- Developing or testing this chart locally: [CONTRIBUTOR_GUIDE.md](CONTRIBUTOR_GUIDE.md)