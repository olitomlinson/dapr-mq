...
export const queueApi = {
  async enqueue(queueId: string, request: EnqueueRequest): Promise<EnqueueResponse> {
    const response = await fetch(`${API_BASE}/queue/${queueId}/enqueue`, {
      method: 'POST',
  ...
  async dequeue(queueId: string, count: number = 1): Promise<DequeueResponse | null> {
    const response = await fetch(`${API_BASE}/queue/${queueId}/dequeue`, {
      method: 'POST',
      headers: {
  ...
  async dequeueLocked(queueId: string, count: number = 1, ttlSeconds: number = 30, allowCompeting: boolean = false): Promise<DequeueLockedResponse | null> {
    const response = await fetch(`${API_BASE}/queue/${queueId}/dequeue`, {
      method: 'POST',
      headers: {
        'require-ack': 'true',
  ...
  async registerSink(queueId: string, request: RegisterSinkRequest): Promise<RegisterSinkResponse> {
    const response = await fetch(`${API_BASE}/queue/${queueId}/sink/http/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(request),
    });
  ...
  async unregisterSink(queueId: string): Promise<void> {
    const response = await fetch(`${API_BASE}/queue/${queueId}/sink/http/unregister`, {
      method: 'POST',
    });
