import type { DequeuedMessage, WiremockRequest } from '../types/queue';
...
export const MessagesList = ({
  ...
  if (!sinkUrl) {
    return (
      <div className="card">
        <h3>Dequeued Messages ({messages.length})</h3>
        {messages.length === 0 ? (
  ...
  return (
    <div className="card">
      <h3>Messages</h3>
      <TabContainer dequeuedCount={messages.length} wiremockCount={wiremockMessageCount}>
        {/* Tab 1: Messages */}
        <div>
          {messages.length === 0 ? (
            <p style={{ fontSize: '0.9em', color: '#666', fontStyle: 'italic' }}>
