...
Build Docker image and run integration tests for DaprMQ
...
build_docker_image() {
    ...
    log_info "Context: $SCRIPT_DIR/server"
...
run_integration_tests() {
    ...
    log_info "Test project: server/tests/DaprMQ.IntegrationTests"
    ...
    local test_args=(
        "test"
        "tests/DaprMQ.IntegrationTests/DaprMQ.IntegrationTests.csproj"
    ...
    if DAPRMQ_TEST_QUEUE_ID="$QUEUE_ID" ENABLE_CONTAINER_LOGS="$ENABLE_CONTAINER_LOGS" dotnet "${test_args[@]}"; then
...
# Run the DaprMQ.Client SDK's own tests (unit + integration) - a separate solution under
# sdks/dotnet, physically outside the server/ tree, so it needs its own dotnet test invocation.
run_client_sdk_tests() {
    log_section "Running DaprMQ.Client SDK Tests"

    log_info "Solution: sdks/dotnet/DaprMQ.Client.sln"
    ...
    if DAPRMQ_TEST_QUEUE_ID="$QUEUE_ID" ENABLE_CONTAINER_LOGS="$ENABLE_CONTAINER_LOGS" dotnet "${test_args[@]}"; then
        log_success "All DaprMQ.Client SDK tests passed!"
    else
        log_error "DaprMQ.Client SDK tests failed"
        exit 1
    fi
...
# Main execution
main() {
    log_section "DaprMQ Build and Test"
    ...
    if [[ "$SKIP_TESTS" == "false" ]]; then
        ...
        log_success "DaprMQ.Client SDK tests: PASSED"
    fi
}
