...
**Key Characteristics:**
- **Lock-Based Safety**: Only locked messages can be moved to DLQ (requires prior DequeueLocked)
- **Atomic Operation**: Item enqueued to DLQ BEFORE being removed from main queue (prevents data loss)
...
2. **Item Retrieval**: Use `Peek()` to read locked item data (for DLQ enqueue)
3. **Enqueue to DLQ**: Create DLQ actor proxy and enqueue item with original priority
4. **Remove Item**: Dequeue item from main queue and update metadata
5. **Void Lock**: Remove lock state ONLY after successful DLQ enqueue and item removal
...
{
    public string LockId { get; init; }  // Lock ID from DequeueLocked
}
```
...
| `DLQ_ENQUEUE_FAILED` | 500 Internal | INTERNAL | Failed to enqueue to DLQ actor |
...
```bash
# 1. Enqueue message to queue
curl -X POST http://localhost:5000/queue/orders/enqueue \
  -H "Content-Type: application/json" \
  -d '{"items": [{"item": {"orderId": 123, "customer": "Alice"}, "priority": 1}]}'

# 2. Dequeue with acknowledgement
curl -X POST http://localhost:5000/queue/orders/dequeue \
  -H "require_ack: true" \
  -H "ttl_seconds: 30"
...
# 5. Inspect DLQ
curl -X POST http://localhost:5000/queue/orders-deadletter/dequeue \
  -H "require_ack: false"
...
// Dequeue with acknowledgement
var dequeueResult = await actor.DequeueLocked(new DequeueLockedRequest { TtlSeconds = 30 });

if (dequeueResult.ItemJson != null)
{
    ...
    while (retryCount < maxRetries)
    {
        ...
        {
            // Process message
            await ProcessOrder(dequeueResult.ItemJson);
...
// Enqueue urgent priority 0 message
await client.EnqueueAsync(new EnqueueRequest
{
    QueueId = "alerts",
    Items =
    {
...
var lockId = dequeueResponse.Success.LockId;
...
// If we later enqueue priority 1 items to DLQ, priority 0 items will dequeue first
```
...
public async Task<DeadLetterResponse> DeadLetter(DeadLetterRequest request)
{
    ...
    // 3. Enqueue to DLQ actor with original priority
    ...
    var enqueueResult = await _actorInvoker.InvokeMethodAsync<EnqueueRequest, EnqueueResponse>(
        ...
        "Enqueue",
        new EnqueueRequest { ItemJson = itemJson, Priority = lockData.Priority });

    if (!enqueueResult.Success) return DLQ_ENQUEUE_FAILED;
    ...
    // 5. Void lock (only after successful DLQ enqueue and item removal)
...
1. **Peek Then Dequeue**: Use `Peek()` to read item data for DLQ enqueue, then `Dequeue()` to remove it after successful enqueue. This ensures data is available for DLQ before removal.

2. **Atomicity**: Enqueue to DLQ FIRST, remove item from main queue SECOND, void lock THIRD. If DLQ enqueue fails, lock remains active and item stays in queue for retry or expiration.
...
```bash
# Check DLQ depth via Dequeue operations
curl -X POST http://localhost:5000/queue/my-queue-deadletter/dequeue
...
1. **Manual Inspection**: Operators dequeue items from DLQ to investigate root cause
...
- `Dequeue()` works as before (no acknowledgement)
- `DequeueLocked()` enables new locking behavior
...
- **Message Acknowledgement**: DLQ requires DequeueLocked (lock-based acknowledgement)
