...
The Message Acknowledgement feature adds at-least-once delivery semantics to the QueueActor by implementing a lock-based acknowledgement system. When enabled, dequeued messages are held in a locked state until explicitly acknowledged or until the TTL expires, preventing message loss if consumers fail during processing.

**Key Characteristics:**
- **Optional Acknowledgement**: Backward compatible - original Dequeue method unchanged
- **Lock-Based Protection**: Prevents concurrent dequeues while waiting for acknowledgement
...
**Parameters:**
- `request` (DequeueLockedRequest): Contains:
    - `TtlSeconds` (int, optional): Lock TTL in seconds (default: 30, range: 1-300)

**Returns:**
```csharp
DequeueLockedResponse
{
    ItemsJson: List<string>,       // Dequeued JSON items
...
4. Dequeues items while tracking priority metadata (doesn't use internal Dequeue method)
...
var result = await actor.DequeueLocked(new DequeueLockedRequest { TtlSeconds = 60 });
...
### Existing Method: Dequeue (Unchanged)

The original `Dequeue` method remains unchanged for backwards compatibility:
...
### Updated Endpoint: POST /queue/{queue_id}/dequeue
...
|----------|-------------|-----------|
| Dequeue without ack (success) | 200 OK | Standard success |
| Dequeue with ack (success) | 200 OK | Standard success |
| Dequeue with ack (queue locked) | 423 Locked | Resource temporarily locked |
...
### Example 2: Lock Prevents Concurrent Dequeues

```bash
# First dequeue with ack
curl -X POST "http://localhost:8002/queue/test/dequeue" \
  -H "require_ack: true"
# Response: {"locked": true, "lock_id": "xyz789", ...}

# Second dequeue attempt (while locked)
curl -X POST "http://localhost:8002/queue/test/dequeue" \
  -H "require_ack: false"
...
curl -X POST "http://localhost:8002/queue/test/acknowledge" \
  -H "Content-Type: application/json" \
  -d '{"lock_id": "short123"}'
# Response (HTTP 410): {"success": false, "error_code": "LOCK_EXPIRED"}

# Next dequeue returns the item again
curl -X POST "http://localhost:8002/queue/test/dequeue" \
  -H "require_ack: true"
...
```bash
# Regular dequeue (no acknowledgement)
curl -X POST "http://localhost:8002/queue/test/dequeue" \
  -H "require_ack: false"
...
### 1. Dequeue While Locked
**Scenario:** DequeueLocked called while another lock is active
**Result:** Returns 423 with lock expiration time, no items dequeued
...
**Scenario:** DequeueLocked on empty queue
...
**Result:** Items automatically returned to their **original priority queues**, next dequeue retrieves them in priority order
...
**Scenario:** DequeueLocked with single item
...
**DequeueLocked:**
- Same as regular Dequeue, plus 1 additional write for lock
...
- Triggered on next DequeueLocked
...
- Concurrent dequeue prevention (423 status)
...
**Run tests:**
```bash
dotnet test --filter "FullyQualifiedName~DequeueLocked"
...
- Original `Dequeue` method unchanged
...
Items from expired locks are returned to their original priority queues using the Enqueue method:
...
if (lockData.ContainsKey("items_json"))
{
    ...
    foreach (var itemJson in itemsJson)
    {
        ...
        {
            await Enqueue(new EnqueueRequest
            {
